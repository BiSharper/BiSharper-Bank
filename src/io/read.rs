...
use crate::{BankFileMeta, BankMime, BankReadOptions};
...
//                 None => BankArchive::normalize_path(filename),
//                 Some(p) => BankArchive::normalize_path(p)
//             };
//             let buffer_start = reader.stream_position()?;
//
//             // let entries: HashMap<GfsPath, BankFile> = metas.drain().map(|(mut name, mut entry)| {
...
//             //     return (BankArchive::normalize_path(&*name), BankFile::create(entry, Arc::from(data.as_slice())));
//             // }).collect();
//
//             (metas, properties, root)
//         };
//         todo!();
//     }
//
